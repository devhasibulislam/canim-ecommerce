...
const ListStore = () => {
  ...
  const { data: storesData, isLoading: displayingStores } =
    useDisplayStoresQuery({
      page,
      limit,
    });
  const [removeStore, { isLoading: removingStore }] = useRemoveStoreMutation();

  const stores = storesData?.data || [];
  const count = storesData?.count || 0;
